		...
		<div class="adminLeft">
			...
			<ul>
				...
				<li><a id="member_center_menu_bbinInfo_record"
					href="/yingJiaProfit/toFrontPersonage/tiyanjin"  class="select"><em
								...
								<tbody>
									...
									<c:forEach items="${listAll }" var="s">
										<tr>
											...
											<td> 
											 <c:if test="${s.subject.status==1 }">募集中</c:if>
                                             <c:if test="${s.subject.status==0 }">已完成</c:if>
                                            </td> 	
											<td>${s.create_date }</td>
										</tr>
									</c:forEach>	
